...
public class FloatingTextFlyweight : Flyweight
{
    ...
    public void AnimateFloatingText(Vector3 position)
    {
        transform.position = position + _spawnOffset;
        _sequence?.Kill(true);
        _sequence = DOTween.Sequence();
        _sequence.Append(transform.DOScale(0.35f, 0.5f).SetEase(Ease.InQuint))
        ...
        _sequence.Append(transform.DOScale(0f, 0.1f).SetEase(Ease.InQuad))
            .OnComplete(Despawn);
    }
