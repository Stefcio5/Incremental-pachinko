...
namespace BreakInfinity
{
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        {
            if (mantissa >= 1 && mantissa < 10 || !IsFinite(mantissa)) return new BigDouble(mantissa, exponent);
            if (IsZero(mantissa)) return Zero;
        ...
        public static BigDouble E = new BigDouble(Math.E);
        public static BigDouble Pi = new BigDouble(Math.PI);
        ...
        public static BigDouble Parse(string value)
        {
            if (value.IndexOf('e') != -1)
            {
                ...
                var mantissa = double.Parse(parts[0], new CultureInfo("en-US"));
                var exponent = long.Parse(parts[1], new CultureInfo("en-US"));
                return Normalize(mantissa, exponent);
            }

            if (value == "NaN") return NaN;

            var result = new BigDouble(double.Parse(value, new CultureInfo("en-US")));
        ...
        {
            try { return Math.Sign(value.Mantissa); }
            catch { return 0; }
        }
        ...
        public static BigDouble Add(BigDouble left, BigDouble right)
        {
            ...
            return Normalize(
                Math.Round(1e14 * bigger.Mantissa +
                           1e14 * smaller.Mantissa * PowersOf10.Lookup(smaller.Exponent - bigger.Exponent)),
                bigger.Exponent - 14);
        }
        ...
        public int CompareTo(object other)
        {
            ...
            if (other is BigDouble) return CompareTo((BigDouble)other);
        ...
        public bool Equals(BigDouble other) => !IsNaN(this) && !IsNaN(other) && (AreSameInfinity(this, other) || Exponent == other.Exponent && AreEqual(Mantissa, other.Mantissa));
        ...
        public static double AbsLog10(BigDouble value) => value.Exponent + Math.Log10(Math.Abs(value.Mantissa));

        public static double Log10(BigDouble value) => value.Exponent + Math.Log10(value.Mantissa);

        public static BigDouble BigLog10(BigDouble value) => value.Exponent + Math.Log10(value.Mantissa);
        ...
        public static BigDouble Pow10(double power) => IsInteger(power) ? Pow10((long)power) : Normalize(Math.Pow(10, power % 1), (long)Math.Truncate(power));

        public static BigDouble Pow10(long power) => new BigDouble(1, power);

        public static BigDouble Pow(BigDouble value, BigDouble power) => Pow(value, power.ToDouble());
        public static BigDouble Pow(BigDouble value, long power) => Is10(value) ? Pow10(power) : Normalize(Math.Pow(value.Mantissa, power), value.Exponent * power);
        ...
        private static bool Is10(BigDouble value) => value.Exponent == 1 && value.Mantissa - 1 < double.Epsilon;
        ...
        public static BigDouble Cbrt(BigDouble value)
        {
            ...
            if (mod != 0) return Normalize(newmantissa * 4.6415888336127789, (long)Math.Floor(value.Exponent / 3.0));

            return Normalize(newmantissa, (long)Math.Floor(value.Exponent / 3.0));
        }

        public static BigDouble Sinh(BigDouble value) => (Exp(value) - Exp(-value)) / 2;

        public static BigDouble Cosh(BigDouble value) => (Exp(value) + Exp(-value)) / 2;

        public static BigDouble Tanh(BigDouble value) => Sinh(value) / Cosh(value);

        public static double Asinh(BigDouble value) => Ln(value + Sqrt(Pow(value, 2) + 1));

        public static double Acosh(BigDouble value) => Ln(value + Sqrt(Pow(value, 2) - 1));
        ...
        private static class PowersOf10
        {
            ...
            static PowersOf10()
            {
                ...
                for (var i = 0; i < Powers.Length; i++)
                    Powers[index++] = double.Parse("1e" + (i - IndexOf0), new CultureInfo("en-US"));
            }
    ...
    public static class BigMath
    {
        ...
        {
            if (Random.NextDouble() * 20 < 1) return BigDouble.Normalize(0, 0);
